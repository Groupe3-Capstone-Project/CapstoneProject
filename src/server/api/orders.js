  ...
  try {
    ...
    if (!req.user.isAdmin && userCart.userId !== req.user.id) {
      return res
        ...
        .json({
          message:
            "Access denied, current user does not match cart user id, or is not admin",
        });
    }
    // console.log("if cart: ",userCart)

    // Check if the product is already in the cart
    const existingCartItem = userCart.cart_items.find(
    ...
    try {
      ...
      if (!req.user.isAdmin && toBeCompleted.userId !== req.user.id) {
        return res
          ...
          .json({
            message:
              "Access denied, current user does not match order user id, or is not admin",
          });
      }
